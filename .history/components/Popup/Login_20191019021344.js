...
const Login = (props) => {
    ...
    return (
        ...
        <Modal isOpen={props.show} toggle={props.onHide} animation={true}>
            ...
            <ModalFooter>
                {_.isEmpty(state.dataLogin) ?
                    <><Button type="button" onClick={() => props.getOtp({ username: statusername, password: password, shopId: data.id })} className="btn btn-solid-primary btn--s btn--inline" >Đăng nhập</Button></>
