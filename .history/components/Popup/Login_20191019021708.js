...
const Login = (props) => {
    ...
    return (
        ...
        <Modal isOpen={props.show} toggle={props.onHide} >
            ...
            <ModalFooter>
                {_.isEmpty(state.dataLogin) ?
                    <><Button type="button" onClick={() => props.getOtp({ username: state.username, password: state.password, shopId: sate.data.id })} className="btn btn-solid-primary btn--s btn--inline" >Đăng nhập</Button></>
